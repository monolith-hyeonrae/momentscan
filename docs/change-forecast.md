...
> **비목표 추가 (2026-07-20)**: 이미지-1장 평가 레인(사진→recipe/프리뷰) — 디자인팀의
> 간헐 요청이 실재하나("이 사람 출력 테스트"), 가끔-수요라 선제 구축 안 함.
> 재개 조건 = 요청 도착 시 lk-preview 위 소형 레인. 사진 픽스처는 우산 에셋
> 홈 보관(파기 금지 — mb-aedel D5 개정).

> **벽 신설 (2026-07-20, user 정식화): "1차 표현 vs 도메인 표현"** — likeness =
> 그 사람다움의 1차 표현(도메인-중립, momentscan 본질 산출) / recipe→shape key =
> 도메인 표현(특정 소비자[디자이너 리그]의 좌표계 — 캘리 range·키 사상·gain 전부).
> momentscan은 기본적으로 도메인 표현을 제공하지 않는다 — **3D 제품용 변환기가
> 어댑터처럼 부착**되어 출력을 변경한다(집행=egress 제외, 의존=단방향 recipe→likeness).
> 인용 축: 소비자 교체(리그 교체·FLAME·diffusion 개인화 — 1차 표현은 불변이어야).
> 신규 도메인 요구는 어댑터 어휘(B1 계약)의 진화로만. 정본 메모=likeness-primary-vs-domain.
