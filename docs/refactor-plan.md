...
·가림 프로브 `scratchpad_emb_occlusion.py`):
   ...
   **맥락적 sort**" — 존 시스템 범주화 제안은 이 논거로 철회(정렬 축의 범주 구겨넣기).
   ...
   → **백화=영상 채널 이관(v0.30.2, user 분류 판정)**: "백화 판단이 빛 채널에 필요한가
   — 노출 비정상은 영상 채널에서" — 분업 확정: **빛=얼굴에 존재하는 조명의 물리 상태
   (백화 프레임도 '빛이 강함'은 참) / 영상=카메라 기록의 온전성(백화=픽셀 포화=기록
   실패)**. exposure entropy validity 게이트·clip_hi와 같은 가족. 백화 배지를 빛
   태그→영상 탭으로 이사, 검증된 서명(lmr≥205∧chr≤45)은 **영상 채널 1층 순회의 첫
   계기 씨앗**으로 예약.
   → **밝기 정렬의 자=타원 중앙값(v0.30.3, user 반례 f224·f454·f473)**: "f454가 f473
   보다 어두워 보이는데 위에 정렬" — 해부: 배포 lmr 3자 동률(208.4±0.1)=타이브레이크
   노이즈. **기전=앵커-가중 평균의 백화 포화**: 이마·볼 앵커 지역이 백화 고원(~208)
   에서 먼저 똑같이 날아가 프레임 간 차이를 지움 — 지각이 보는 차이는 타원 전체
   분포에 잔존(중앙값 f473 241 vs f454 212, Δ29). **직무 분리**: 게이트 자(lt, 앵커
   가중=오염 회피 설계)는 유지 / **정렬 자=타원 전체 중앙값 lmd**(눈·눈썹=소면적이라
   중앙값 강건, 지각 정합). skin_sv 말미-추가로 언패킹 불변. 교훈: **같은 "밝기"도
   게이트(강건 스크리닝)와 정렬(지각 순위)은 다른 자를 요구한다** — 구분/정렬 두
   동사 원칙의 측정판.
   → **⚠미결(2026-07-27 마감)**: lmd 교체 후에도 "밝기 정렬이 지각과 안 맞는 사례
   잔존"(user). 내일 첫 수=반례 프레임 수집 → 자 후보 비교. **유력 용의자: skin_sv의
   밝기=HSV V(=RGB max)** — 지각 휘도(luma 0.299R+0.587G+0.114B)와 달리 채도 픽셀을
   과대 판독(내 재현[gray luma 중앙값 236/212/241]과 배포[V 중앙값 242/232/242]의
   차이도 이것). 후보: luma 중앙값·p75·상위-트림 평균. 부차 용의자: 크롭 배경 대비
   착시·얼굴 면적.
