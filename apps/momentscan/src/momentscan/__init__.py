...
The package layout mirrors the architecture (2026-07-02 restructure):
  extraction/ L0-L1 extraction nodes (heavy model backends live in the isolated
              ``plugins/`` workspace packages behind the FeatureSource port)
  domains/    L2 signal-domain policy (fusion · quantizers · thresholds)
  gates.py    L3 the declared gate ladder → gate_trace
  products/   L4 engines (likeness · portrait · select · highlight_lang)
  surface/    frontend · verify/ replay-freshness-eval-graph
