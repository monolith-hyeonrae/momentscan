...
def _owner_style_mask(m: np.ndarray) -> np.ndarray:
    """subject-소유 스타일 픽셀만. 씨앗 = 중심-최근접 얼굴 성분; cloth/안경류는
    얼굴∪소유자-목 직접-인접, 모자류는 +소유자-헤어 인접. 얼굴 없음 → 빈 마스크."""
...
def _color_identity(frames_bgr: list[np.ndarray], seg_model, device) -> dict | None:
    """방문-집계 의상 팔레트 → {primary, secondary, highlight, palette_diversity}.
    각 색 = {lab, hex, area}; highlight = 면적>5% 중 최고 채도 (없으면 'neutral')."""
    import torch
    ...
    return {"primary": _c(int(order[0])), "secondary": _c(int(order[1])),
            ...
            "n_px": int(sizes.sum()), "n_frames": len(pool)}
...
def extract_fashion(out_root, clip_id: str, *, fps: int = 6) -> dict:
    ...
    for s in manifest["subjects"]:
        ...
        subjects.append({"subject_id": int(s["subject_id"]), "role": s["role"],
                         ...
                         "color_identity": _color_identity(frames, seg, device)})
