...
렌더(graph)가 한 지붕: 트리가 "무엇이 어떻게 도는가"에 답한다 (접수 #1·#4·#6).
...
gates.py 는 A″에서 perception 으로 떠났다 — 판정(admit/reject/route)은 관측 계층의
이진 판별기지 순수 실행 기계가 아니다(user 판정). 이 패키지는 이제 순수 기계만 남아
졸업 운명과 정합: 전원 visualpath(L3) ArtifactNode 졸업석 예약 — 이 경계가 R16/R17의 절단선.
"""
